...
class BaseModel:
    def __init__(self):
        ...
        self.model.client.api_url = 'https://api-inference.huggingface.co/models/mistralai/Mixtral-8x7B-Instruct-v0.1'
        self.model.invoke('foo bar')
        self.memory = ConversationBufferMemory(
            memory_key='chat_history', return_messages=True
        )
        self.prompt = (
            'Always check chat history before answer.'
    ...
    async def answer(self, question):
        ...
        if len(r) > 30:
            self.drop_memory()
            return (
                'Free AI memory was was full. AI continue without previous messages. '
        ...
        if chat_memory:
            if self.document:
                ...
                prompt = self.prompt.format(
                    chat_history=chat_memory, user_input=question
                )
            ...
            prompt = self.prompt.format(
                chat_history='Without chat history', user_input=question
            )
            answer = self.model(prompt)
            self.add_to_memory(question, answer)
        return answer
